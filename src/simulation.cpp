	...
	m_Buffer(new BufferManager)
{
	...
	cmd.add(nb_alleles);*/
	TCLAP::UnlabeledMultiArg<double> frequences("frequences", "Frequence of each allele in the population", false, "double");
	cmd.add(frequences);
...
void Simulation::run() {
	...
	for(int r(0); r < mNbRepeats; r++) {
		...
		for(int i(0); i < mDuration; i++) {
			...
			// ajout des fréquences au buffer
			m_Buffer->print_in(frequences);
		}
		// here should clear buffer, get its content, and pass it to graphics (do it for each repeat)
		// std::string buffer_content(m_Buffer->clear());
	}
}
...
void Simulation::initialize(int nb_repeats, int duration, int pop_size, const std::vector<double> &frequences, std::string buffer_name) {
	...
	std::vector<double> vec_freq(frequences);
	if(vec_freq.size() == 0)
		vec_freq = SIMULATION_DEFAULT_FREQUENCES;
	double sum(0);
	for(double d : vec_freq) {
