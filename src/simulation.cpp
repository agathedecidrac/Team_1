...
Simulation::Simulation(int argc, char** argv) {
	...
	cmd.add(nb_alleles);
	* */
	TCLAP::MultiArg<double> frequences("f", "frequences", "Frequence of each allele in the population", false, "double");
	cmd.add(frequences);
...
void Simulation::initialize(int nb_repeats, int duration, int pop_size, std::vector<double> frequences) {
	if(nb_repeats < 1) throw std::string("Invalid number of repeats");
	if(pop_size < 1) throw std::string("Invalid population size"); 
	double sum(0);
	for(double d : frequences) {
		if(d < 0.0 or d > 1.0) throw std::string("Invalid frequences (should be in the range 0-1)"); // error...
		sum += d;
	}
	if(sum == 0) throw std::string("You should specify at least 1 frequence");
	if(sum > 1.0) throw std::string("Frequences sum to more than 1");
	...
	for(double d : frequences)
		mCurrentAlleles.push_back( d * mPopulationSize );
}
